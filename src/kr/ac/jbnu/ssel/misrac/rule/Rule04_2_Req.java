 ...
 * third character (e.g. ??- represents a '~' (tilde) character and ??) represents a ']'). 
 * They can cause accidental confusion with other uses of two question marks. For example the string
 * 
...
//		String trigraphPostfix = "=()/'<!>-";
		...
		{
			if( literalExpression.indexOf(c) != -1) // char contains trigraph postfix. 
			{
				String msg = MessageFactory.getInstance().getMessage(3601);
				violationMsgs.add(new ViolationMessage(this, getRuleID() + ":" + msg + ":"+ literalExpression +"("+expression.hashCode()+")", expression));
